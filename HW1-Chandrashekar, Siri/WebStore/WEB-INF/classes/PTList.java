...
public class PTList extends HttpServlet {
	...
	/* Checks the Tablets type whether it is microsft or apple or samsung */
		...
		{
			if(CategoryName.equalsIgnoreCase("Dog Tracker"))
			{
				for(Map.Entry<String,Pettracker> entry : SaxParserDataStore.pettrackers.entrySet())
				{
				  if(entry.getValue().getName().equals("Dog Tracker"))
				  {
		...
		for (Map.Entry<String, Pettracker> entry : hm.entrySet()) {
			...
			pw.print("<li id='item'><img src='images/pettracker/"
					+ pettracker.getImage() + "' alt='' /></li>");
