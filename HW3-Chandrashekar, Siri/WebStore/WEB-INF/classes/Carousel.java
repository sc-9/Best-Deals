...
public class Carousel {
    ...
    public String carouselfeature(Utilities utility) {
        ...
        if (CategoryName == null) {
            hm1.putAll(SaxParserDataStore.phones);
            name = "";
        }
        int l = 0;
        for (OrderItem oi : utility.getCustomerOrders()) {
            //System.out.println(oi.getName());
            //if (hm1.containsKey(oi.getName())) {
                //oi.getName().replace("_"," ");
                myCarousel = "myCarousel" + l;

                sb.append("<div id='content'><div class='post'><h2 class='title meta'>");
                sb.append("<a style='font-size: 24px;'>" +  oi.getName().replace("_"," ") + " Accessories</a>");
				...
				*/
                ...
                Phone phone1 = hm1.get(oi.getName());
                ...
                for (Map.Entry<String, Accessory> entry : hm.entrySet()) {
                    ...
                    sb.append("<li id='item'><img src='images/accessory/" + accessory.getImage() + "' alt='' /></li>");
                    sb.append("<li><form method='post' action='Cart'>" +
                            "<input type='hidden' name='name' value='" + accessory.getPrice() + "'>" +
                            "<input type='hidden' name='type' value='accessory'>" +
                            "<input type='hidden' name='maker' value='" + CategoryName + "'>" +
                            "<input type='hidden' name='access' value='" + accessory.getName() + "'>" +
                            "<input type='submit' class='btnbuy' value='Buy Now'></form></li>");
                    sb.append("<li><form method='post' action='WriteReview'>" + "<input type='hidden' name='name' value='" + accessory + "'>" +
                            "<input type='hidden' name='type' value='accessory'>" +
                            "<input type='hidden' name='maker' value='" + CategoryName + "'>" +
                            "<input type='hidden' name='access' value='" + accessory.getName() + "'>" +
                            "<input type='submit' value='WriteReview' class='btnreview'></form></li>");
                    sb.append("<li><form method='post' action='ViewReview'>" + "<input type='hidden' name='name' value='" + accessory + "'>" +
                            "<input type='hidden' name='type' value='accessory'>" +
                            "<input type='hidden' name='maker' value='" + CategoryName + "'>" +
                            "<input type='hidden' name='access' value='" + accessory.getName() + "'>" +
